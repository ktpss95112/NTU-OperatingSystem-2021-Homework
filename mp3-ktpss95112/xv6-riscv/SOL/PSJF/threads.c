...
void schedule(void){
    ...
    int min_id = 1000000;
    int min_remain = 1000000;

    struct thread *tmp_thread = current_thread;
    while (tmp_thread != NULL) {
        // for every thread, check the remaining time
        if (!tmp_thread->is_exited)
        if (!tmp_thread->is_exited && tmp_thread->remain_execution_time <= min_remain) {
            if (tmp_thread->remain_execution_time < min_remain || tmp_thread->ID < min_id) {
                target = tmp_thread;
                min_id = tmp_thread->ID;
                min_remain = tmp_thread->remain_execution_time;
            }
        }
